...
class Twt_box
{
	...
	function publishTweet($criteria=array()) {
		...
		$in_reply_to_status_id = $criteria['in_reply_to_status_id'];
	...
	function postDataToAPI($criteria=array()) {
		...
		$code = $tmhOAuth->request('POST', $tmhOAuth->url('1.1/'.$connection.'.json'), $params);
		$response = json_decode($tmhOAuth->response['response']);
		return $response;
	}
	...
	function getDataFromAPI($criteria=array()) {
		...
		// echo $tmhOAuth->url('1.1/'.$connection.'.json');
		// exit;
