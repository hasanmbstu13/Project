...
class Purchases extends MY_Controller
{
    ...
    /* Withdraws */
    function withdraws($id = NULL)
    {

        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['page_title'] = lang('withdraws');
        $bc = array(array('link' => site_url('purchases'), 'page' => lang('purchases')), array('link' => '#', 'page' => lang('withdraws')));
        $meta = array('page_title' => lang('withdraws'), 'bc' => $bc);
        $this->page_construct('purchases/withdraws', $this->data, $meta);

    }

    function get_withdraws($user_id = NULL)
    {

        $detail_link = anchor('purchases/withdraw_note/$1', '<i class="fa fa-file-text-o"></i> ' . lang('withdraw_note'), 'data-toggle="modal" data-target="#myModal2"');
        $edit_link = anchor('purchases/edit_withdraw/$1', '<i class="fa fa-edit"></i> ' . lang('edit_withdraw'), 'data-toggle="modal" data-target="#myModal"');
        $delete_link = "<a href='#' class='po' title='<b>" . $this->lang->line("delete_withdraw") . "</b>' data-content=\"<p>"
            . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . site_url('purchases/delete_withdraw/$1') . "'>"
            . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i> "
            . lang('delete_withdraw') . "</a>";
        $action = '<div class="text-center"><div class="btn-group text-left">'
            . '<button type="button" class="btn btn-default btn-xs btn-primary dropdown-toggle" data-toggle="dropdown">'
            . lang('actions') . ' <span class="caret"></span></button>
        <ul class="dropdown-menu pull-right" role="menu">
        ...
        $this->load->library('datatables');
        $this->datatables
            ->select($this->db->dbprefix('withdraws') . ".id as id, date, reference, amount, note, CONCAT(" . $this->db->dbprefix('users') . ".first_name, ' ', " . $this->db->dbprefix('users') . ".last_name) as user, attachment", FALSE)
            ->from('withdraws')
            ->join('users', 'users.id=withdraws.created_by', 'left')
            ->group_by('withdraws.id');

        if ( ! $this->Admin) {
            $this->datatables->where('created_by', $this->session->userdata('user_id'));
        }
        $this->datatables->add_column("Actions", "<div class='text-center'><div class='btn-group'><a onclick=\"window.open('".site_url('purchases/withdraw_note/$1')."', 'pos_popup', 'width=900,height=600,menubar=yes,scrollbars=yes,status=no,resizable=yes,screenx=0,screeny=0'); return false;\" href='#' title='".lang('withdraw_note')."' class='tip btn btn-primary btn-xs'><i class='fa fa-file-text-o'></i></a> <a href='" . site_url('purchases/edit_withdraw/$1') . "' title='" . lang("edit_withdraw") . "' class='tip btn btn-warning btn-xs'><i class='fa fa-edit'></i></a> <a href='" . site_url('purchases/delete_withdraw/$1') . "' onClick=\"return confirm('" . lang('alert_x_withdraw') . "')\" title='" . lang("delete_withdraw") . "' class='tip btn btn-danger btn-xs'><i class='fa fa-trash-o'></i></a></div></div>", "id");
        $this->datatables->unset_column('id');
        echo $this->datatables->generate();
    }
    ...
    function add_withdraw()
    {
        ...
        } elseif ($this->input->post('add_withdraw')) {
            $this->session->set_flashdata('error', validation_errors());
            redirect($_SERVER["HTTP_REFERER"]);
        }

        if ($this->form_validation->run() == true && $this->purchases_model->addWithdraw($data)) {

            $this->session->set_flashdata('message', lang("withdraw_added"));
            redirect('purchases/withdraws');

        } else {

            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['page_title'] = lang('add_withdraw');
            $bc = array(array('link' => site_url('purchases'), 'page' => lang('purchases')), array('link' => site_url('purchases/withdraws'), 'page' => lang('withdraws')), array('link' => '#', 'page' => lang('add_withdraw')));
            $meta = array('page_title' => lang('add_withdraw'), 'bc' => $bc);
            $this->page_construct('purchases/add_withdraw', $this->data, $meta);

        }
    }

    function withdraw_note($id = NULL)
    {
        if ( ! $this->Admin) {
            if($withdraw->created_by != $this->session->userdata('user_id')) {
                $this->session->set_flashdata('error', lang('access_denied'));
                redirect(isset($_SERVER["HTTP_REFERER"]) ? $_SERVER["HTTP_REFERER"] : 'pos');
            }
        }

        $withdraw = $this->purchases_model->getWithdrawByID($id);
        $this->data['user'] = $this->site->getUser($withdraw->created_by);
        $this->data['withdraw'] = $withdraw;
        $this->data['page_title'] = $this->lang->line("withdraw_note");
        $this->load->view($this->theme . 'purchases/withdraw_note', $this->data);

    }

    function edit_withdraw($id = NULL)
    {
        if ( ! $this->Admin) {
            $this->session->set_flashdata('error', lang('access_denied'));
            redirect('pos');
        ...
        } elseif ($this->input->post('edit_withdraw')) {
            $this->session->set_flashdata('error', validation_errors());
            redirect($_SERVER["HTTP_REFERER"]);
        }


        if ($this->form_validation->run() == true && $this->purchases_model->updateWithdraw($id, $data)) {
            $this->session->set_flashdata('message', lang("withdraw_updated"));
            redirect("purchases/withdraws");
        } else {

            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['withdraw'] = $this->purchases_model->getWithdrawByID($id);
            $this->data['page_title'] = lang('edit_withdraw');
            $bc = array(array('link' => site_url('purchases'), 'page' => lang('purchases')), array('link' => site_url('purchases/withdraws'), 'page' => lang('withdraws')), array('link' => '#', 'page' => lang('edit_withdraw')));
            $meta = array('page_title' => lang('edit_withdraw'), 'bc' => $bc);
            $this->page_construct('purchases/edit_withdraw', $this->data, $meta);

        }
    }

    function delete_withdraw($id = NULL)
    {
        if(DEMO) {
            $this->session->set_flashdata('error', lang('disabled_in_demo'));
            redirect(isset($_SERVER["HTTP_REFERER"]) ? $_SERVER["HTTP_REFERER"] : 'welcome');
        ...
        $withdraw = $this->purchases_model->getWithdrawByID($id);
        if ($this->purchases_model->deleteWithdraw($id)) {
            // if ($withdraw->attachment) {
            //     unlink($this->upload_path . $withdraw->attachment);
            // }
            $this->session->set_flashdata('message', lang("withdraw_deleted"));
            redirect('purchases/withdraws');
        }
    }
