...
class Reports extends MY_Controller
{
    ...
    function get_expenses($start_date, $end_date) {
        ...
        // }
         ...
         // $this->db->order_by($this->db->dbprefix('expenses').'.id', 'DESC');
    ...
    function get_withdraws($start_date, $end_date) {

        // $this->db->select('count(id) as total, sum(COALESCE(amount, 0)) as total_withdraws, SUM(COALESCE(paid, 0)) as paid, SUM(COALESCE(total_tax, 0)) as tax', FALSE)
        ...
        // return FALSE;
         ...
         // $this->db->select(
         //    // 'sum(COALESCE(amount, 0)) as total_withdraws')
         //     $this->db->dbprefix('withdraws').".date, "
         //    .$this->db->dbprefix('withdraws').".reference, "
         //    .$this->db->dbprefix('withdraws').".note, "
         //    .$this->db->dbprefix('withdraws').".created_by, "
         //    .$this->db->dbprefix('withdraws').".amount ")
         $this->db->select('sum(COALESCE(amount, 0)) as total_withdraws')
         ->from('withdraws');
    ...
    function products_history() {
        ...
        $this->data['p_history'] = $this->get_product_history($start_date, $end_date,$product_name);     
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['page_title'] = $this->lang->line("products_history");
        $bc = array(array('link' => '#', 'page' => lang('reports')), array('link' => '#', 'page' => lang('products_history')));
        $meta = array('page_title' => lang('products_history'), 'bc' => $bc);
        $this->page_construct('reports/products_history', $this->data, $meta);

    }

    

    function get_product_history($start_date, $end_date, $product_name) {

        $sql = "SELECT tec_products.code, tec_products.quantity,tec_products.price,tec_products.cost,
                        (SELECT SUM(tec_sale_items.quantity)
                        FROM tec_sale_items ";
        if($start_date || $end_date)
            $sql .= "  LEFT JOIN tec_sales ON tec_sale_items.sale_id = tec_sales.id";  
        $sql .=        " WHERE 1=1 ";
    
        if($start_date) {
           $date = new DateTime($start_date);
           $start_date = $date->format('Y-m-d' ); 
           $sql .= " AND DATE(tec_sales.date) >='".$start_date."'";
        } 

        if($end_date) {
            $date = new DateTime($end_date);
            $end_date = $date->format('Y-m-d' );
            $sql .= " AND DATE(tec_sales.date) <='".$end_date."'";
        }

         $sql .= " AND tec_products.id = tec_sale_items.product_id) AS SOLD,
                    tec_purchases.date
                FROM tec_products
                LEFT JOIN tec_purchase_items
                ON tec_products.id = tec_purchase_items.product_id
                LEFT JOIN tec_purchases
                ON tec_purchases.id = tec_purchase_items.purchase_id ";
        
        $sql .= " WHERE 1=1 ";
       
        if($product_name) 
            $sql .= " AND tec_products.code = '".$product_name."'";
    ...
    function comprehensive_sales_report()
    {
        ...
        $this->db->select(
            ...
            .$this->db->dbprefix('sale_items').".cost, "
        ...
        $this->db->order_by($this->db->dbprefix('sales').'.id', 'DESC');
        ...
        foreach ($query->result() as $row) {
            if($row->id == $id){
                $data[$row->id]['code'][$row->code]['p_code'] = $row->code;  
                $data[$row->id]['code'][$row->code]['p_quantity'] = $row->quantity;
                $data[$row->id]['code'][$row->code]['p_cost'] = $row->cost;                
            } else {
                $data[$row->id]['id'] = $row->id;
                $data[$row->id]['date'] = $row->date;
                $data[$row->id]['grand_total'] = $row->grand_total;
                $data[$row->id]['total_discount'] = $row->total_discount;
                $data[$row->id]['code'][$row->code]['p_code'] = $row->code;  
                $data[$row->id]['code'][$row->code]['p_quantity'] = $row->quantity;
                $data[$row->id]['code'][$row->code]['p_cost'] = $row->cost;
            }
...
*/
        ...
        if($end_date) 
            $this->data['end_date'] = $end_date;  
        $this->data['expenses'] = $this->get_expenses($start_date, $end_date);     
        $this->data['withdraws'] = $this->get_withdraws($start_date, $end_date);     
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['page_title'] = $this->lang->line("comprehensive_sales_report");
        $bc = array(array('link' => '#', 'page' => lang('reports')), array('link' => '#', 'page' => lang('comprehensive_sales_report')));
        $meta = array('page_title' => lang('comprehensive_sales_report'), 'bc' => $bc);
        $this->page_construct('reports/comprehensive_sales_report', $this->data, $meta);
    }
