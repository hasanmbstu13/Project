...
class Purchases_model extends CI_Model
{
    ...
    public function getWithdrawByID($id) {
        $q = $this->db->get_where('withdraws', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function addWithdraw($data = array()) {
        if ($this->db->insert('withdraws', $data)) {
            return true;
        }
        return false;
    }

    public function updateWithdraw($id, $data = array()) {
        if ($this->db->update('withdraws', $data, array('id' => $id))) {
            return true;
        }
        return false;
    }
    ...
    public function deleteWithdraw($id) {
        if ($this->db->delete('withdraws', array('id' => $id))) {
            return true;
        }
        return FALSE;
    }
