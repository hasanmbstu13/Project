...
class Reports_model extends CI_Model
{
	...
	public function getDailySales($year, $month)
	{
		...
		if($q->num_rows() > 0) {
			...
			// Got total cost_price - modification from CT
/*
			$salesQuery2 = "SELECT DATE_FORMAT( ".$this->db->dbprefix('sales').".date,  '%e' ) AS date, COALESCE(sum(".$this->db->dbprefix('sale_items').".cost), 0) as total_cost FROM (".$this->db->dbprefix('sales').")
			LEFT JOIN ".$this->db->dbprefix('sale_items')." ON ".$this->db->dbprefix('sales').".id=".$this->db->dbprefix('sale_items').".sale_id
			WHERE DATE_FORMAT( ".$this->db->dbprefix('sales').".date,  '%Y-%m' ) =  '{$year}-{$month}'
			GROUP BY DATE_FORMAT( ".$this->db->dbprefix('sales').".date,  '%e' )";
			$query2 = $this->db->query($salesQuery2, false);

		    if($query2->num_rows() > 0) {
				foreach (($query2->result()) as $key=>$row) {
					if(array_key_exists($key, $data) && $data[$key]->date)
					$data[$key]->total_cost = $row->total_cost;
				}
			}
*/
			//end of modification from CT
	...
	* Get total costs
	***/
	public function getTotalCost($start, $end)
    {
		// Got total cost_price - modification from CT
		$salesQuery = "SELECT COALESCE(sum(".$this->db->dbprefix('sale_items').".cost*".$this->db->dbprefix('sale_items').".quantity), 0) as total_cost FROM ".$this->db->dbprefix('sale_items').", ".$this->db->dbprefix('sales')." WHERE ".$this->db->dbprefix('sale_items').".sale_id= ".$this->db->dbprefix('sales').".id AND ".$this->db->dbprefix('sales').".date >= '{$start}' and ".$this->db->dbprefix('sales').".date <= '{$end}'";
		$q = $this->db->query($salesQuery, false);
